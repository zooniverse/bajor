...
mkdir -p $AZ_BATCH_NODE_MOUNTS_DIR/$CONTAINER_MOUNT_DIR/models/$TRAINING_JOB_RESULTS_DIR

# move the checkpoint to the models job dir for long terms storage (tracking)
cp -f $best_checkpoint $AZ_BATCH_NODE_MOUNTS_DIR/$CONTAINER_MOUNT_DIR/models/$TRAINING_JOB_RESULTS_DIR/

# overwrite the existing deployed model for use in other services (prediction scripts etc)
cp -f $best_checkpoint $AZ_BATCH_NODE_MOUNTS_DIR/$CONTAINER_MOUNT_DIR/models/zoobot.ckpt
