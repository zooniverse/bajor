...
def save_predictions_to_json(predictions: np.ndarray, image_ids: List[str], label_cols: List[str], save_loc: str):
    ...
    Args:
        predictions (np.ndarray): Zoobot concentrations of shape (galaxy, answer) or (galaxy, answer, 1), where 1 is sample dim.
    ...
    if predictions.ndim == 3:
        assert predictions.shape[2] == 1, f'Attempted to save predictions with shape {predictions.shape} but only support sample_dim=1'
...
def test_save_predictions_to_json():
    ...
    # predictions may in general have sample_dim=1, will automatically squeeze
    predictions = np.expand_dims(predictions, 2)  # now (galaxy, answer, 1)
